...
class MMADiffusionConfig(BaseConfig):
    ...
    Attributes:
        ...
        target_prompts: Concept-specific prompts for the GCG attack to target.
                        ...
                        prompts streamed from the I2P dataset's "sexual" category
                        if not provided (see `i2p_target_limit`).
        i2p_target_limit: Max number of I2P nudity prompts used as GCG attack
                          targets when `target_prompts` is not provided. Each
                          target prompt triggers a full GCG optimisation run, so
                          this bounds the overall attack cost. Default 500
                          (I2P's "sexual" category has 931 prompts total).
    ...
    i2p_target_limit: Optional[int] = 500
