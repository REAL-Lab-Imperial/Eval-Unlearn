...
def load_coco_captions(
    ...
    """
    Stream COCO captions from HuggingFace, without downloading images.

    Returns a DataLoader that yields Dataset batches with:
      - prompts: list of caption strings
    ...
    logger.info(
        "Setting up HF streaming for COCO captions (%s, split=%s)...",
        cfg["repo_id"],
        cfg["split"],
    )
    ...
    def collate_fn(batch):
        ...
        return Dataset(
            prompts=captions,
            metadata={"source": "coco_hf", "repo_id": cfg["repo_id"]},
        )
